...
// 4
void Cylinder::computeFeatures(Eigen::MatrixXd &scene_normal, std::vector<Eigen::Vector3d> &points, std::vector<float> &intensities) {
  bool status;
  ...
  for (int r=0; r<features_size; ++r) {
    ...
    status = features[r].computeFeatures(&grid[r], scene_normal, points, intensities);
...
// 4
void Cylinder::computeFeatures_wt_Trick(Eigen::MatrixXd &scene_normal, std::vector<Eigen::Vector3d> &points, std::vector<float> &intensities, Cylinder *cyl_) {
  bool status;
  int features_size = (int)features.size();
...
void     Cylinder::produceFeaturesRoutine(DataLoader &dl, Cylinder *back_cyl) {
  ...
  computeFeatures(dl.scene_normal, dl.points, dl.intensities);
...
void     Cylinder::OnlineRoutine(DataLoader &dl, Cylinder *back_cyl) {
  ...
  computeFeatures(dl.scene_normal, dl.points, dl.intensities);    // if feature.computeFeatures fails: _still_ UNPREDICTABLE and UNKNOWN_CELL_LABEL
...
uint64_t Cylinder::OnlineRoutineProfile_wt_Trick(DataLoader &dl, Cylinder *back_cyl) {
  ...
  if (level==0)
    ...
    computeFeatures_wt_Trick(dl.scene_normal, dl.points, dl.intensities, back_cyl);
